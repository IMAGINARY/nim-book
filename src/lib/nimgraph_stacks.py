...
def create_heap(dwg, size, items_left, suggestion=None):
    group = dwg.g()
    for i in range(size):
        y = -1 * i * (ITEM_SIZE + ITEM_SPACING)
        removed = (size - i) <= (size - items_left)
        if not removed:
            if suggestion is not None and (items_left - suggestion) <= i < items_left:
                group.add(create_heap_item(dwg, ITEM_SHAPE, ITEM_STROKE, y, ITEM_SIZE, True))
            else:
                group.add(create_heap_item(dwg, ITEM_SHAPE, ITEM_STROKE, y, ITEM_SIZE, False))
    return group
